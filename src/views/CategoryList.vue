...
<br>
    <el-button type="info" @click="addCategory">+添加新的菜品类别</el-button>
    <el-table :data="categoryList" border stripe>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button @click="updateCategory(c,i)"type="success" size="mini">修改</el-button>
          <el-button @click="deleteCategory(c,i)"type="danger" size="mini">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods:{
    deleteCategory(c,i){
      this.$confirm("删除操作不可撤销,确定吗？","提示",{type:"warning"}).then(()=>{
        var url=this.$store.state.globalSettings.apoUrl+"/admin/category/"+c.cid;
    ...
    addCategory(){
      this.$prompt("请输入新的菜品类别：","提示",{type:"info"}).then(({value})=>{
        console.log(value);
        //获得用户输入 调用数据API添加到数据库
        var url=this.$state.globalSettings.apiUrl+"/admin/category";
